...
export function profileChecklist(user: Profile | null): ChecklistRow[] {
  return [
    ...
    },
    {
      ...
      value: user?.paymentOnFile ? (user?.cardLast4 ? `Visa ····${user.cardLast4}` : "On file") : undefined,
      required: true,
    },
  ];
}
...
export function isChecklistId(v: string | undefined): v is ChecklistId {
  return !!v && v in SECTION_TITLES;
}
