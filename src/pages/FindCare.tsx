...
function priceLabel(t: Treatment, tx: (s: string) => string) {
  if (t.from === 0) return tx("Covered by most plans");
  return (
    <>
      {tx("From")}{" "}
      <span className="font-semibold text-[color:var(--pp-primary-950)] tnum">${t.from}</span>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        <p className="pp-caps text-[color:var(--pp-violet)]">{tx(t.category)}</p>
        <h3 className="mt-1.5 font-display text-xl font-medium tracking-tight text-[color:var(--pp-primary-950)]">
          {tx(t.name)}
        </h3>
        <p className="mt-1.5 flex-1 text-sm leading-relaxed text-ink-secondary">{tx(t.blurb)}</p>
        <div className="mt-4 flex items-end justify-between gap-3 border-t border-line pt-3">
          <p className="text-sm text-ink-secondary">{priceLabel(t, tx)}</p>
  ...
  return (
    <div>
      <header className="mb-6">
        <p className="pp-caps text-[color:var(--pp-violet)]">{tx("Find care")}</p>
        <h1 className="mt-2 font-display text-3xl font-medium tracking-tight text-[color:var(--pp-primary-950)] md:text-4xl">
          {tx("What would you like help with?")}
        </h1>
        <p className="mt-2 max-w-xl text-base text-ink-secondary">
          {tx(
            "Search a symptom or condition, or browse doctor-led treatments. Assessment online — meds delivered free across Canada.",
          )}
        </p>
      </header>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder={tx('Try "heartburn", "birth control", "blood pressure"…')}
          aria-label={tx("Search symptoms or treatments")}
      ...
      <div className="pp-scroll -mx-1 mt-4 flex gap-2 overflow-x-auto px-1 pb-1" role="group" aria-label={tx("Filter by category")}>
        {cats.map((c) => (
          <button
            key={c}
            type="button"
      ...
      <p className="mt-6 text-sm text-ink-tertiary tnum">
        {filtered.length} {filtered.length === 1 ? tx("treatment") : tx("treatments")}
      </p>
      ...
      {filtered.length === 0 ? (
        <div className="mt-4 rounded-2xl border border-line bg-white px-6 py-14 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-sm text-ink-tertiary">
            {tx("Try a broader term, or message our care team to point you in the right direction.")}
          </p>
          <button
            type="button"
            onClick={() => {
