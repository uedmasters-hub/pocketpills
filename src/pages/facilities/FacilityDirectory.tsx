...
import { listHfDistricts, listHfFacilities, normalizeHfCode, type HfFacility } from "@/lib/hfApi";
import type { ReviewSummary } from "@/lib/reviewsApi";
import { useReviewSummaries } from "@/lib/useReviewSummaries";
import {
  displayFacilityLevel,
  displayFacilityName,
  ensureDemoPublishedFacilities,
  facilityHours,
  getFacilityClaim,
  listPublishedFacilityClaims,
  maskFacilityName,
  shortHfCode,
  subscribeFacilityDirectory,
  type FacilityClaim,
} from "@/lib/facilityDirectory";
import {
  districtSelectOptions,
  nearbyFacilityDistricts,
  normalizeCityName,
  readSavedFacilityDistrict,
  saveFacilityDistrict,
...
function readDirQuery() {
  ...
  return {
    ...
    district: fromUrl || readSavedFacilityDistrict(),
    registered: p.get("registered") === "1",
  };
}

function claimAsFacility(claim: FacilityClaim): HfFacility {
  return {
    hfCode: claim.hfCode,
    name: claim.name,
...
function claimMatchesDistrict(claim: FacilityClaim, district: string) {
  const needle = district.toLowerCase();
  return claim.district.toLowerCase().includes(needle);
}
...
function registeredForDistrict(district: string): FacilityClaim[] {
  const all = listPublishedFacilityClaims();
...
export function FacilityDirectory() {
  ...
  const [rows, setRows] = useState<HfFacility[]>([]);
  ...
  useEffect(() => {
    const unsub = subscribeFacilityDirectory(() => setRev((n) => n + 1));
    ensureDemoPublishedFacilities();
    void listHfDistricts().then((list) => {
  ...
  useEffect(() => {
    ...
    const qs = p.toString();
    const url = qs ? `/facilities?${qs}` : "/facilities";
  ...
  const selectDistrict = (next: string) => {
    const saved = saveFacilityDistrict(next);
    setDistrict(saved);
    setQ("");
    setAppliedQ("");
    setPage(1);
  ...
  const liveReviewIds = useMemo(
    () =>
      gridItems.flatMap((item) =>
        "facility" in item && item.activated
          ? [normalizeHfCode(item.facility.hfCode) || String(item.facility.hfCode)]
          : [],
      ),
    [gridItems],
  );
  const { map: reviewSummaries, ready: reviewsReady } = useReviewSummaries("facility", liveReviewIds);
  ...
  return (
    <div>
      <header className="mb-6 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0 max-w-xl">
          ...
          <p className="mt-2 text-base text-ink-secondary">
            {tx(
              "Every registered hospital, clinic, and lab. Available profiles show hours. Unclaimed cards fade until the facility verifies and claims.",
            )}
          </p>
        </div>
        <form className="w-full max-w-lg lg:pb-0.5" onSubmit={(e) => e.preventDefault()}>
          <p className="mb-1.5 text-right text-sm font-medium text-ink-secondary">{tx("Search facilities")}</p>
          <PageSearchField
            scope="facilities"
            value={q}
            onChange={setQ}
      ...
      <p className="mt-10 text-center text-sm text-ink-tertiary">
        {tx("Are you a registered hospital, clinic, or lab?")}{" "}
        <Link to="/facilities/claim" className="font-medium text-[color:var(--pp-violet)] hover:underline">
...
function DistrictEmptyState({
  ...
  const { tx } = useI18n();
  const nearby = nearbyFacilityDistricts(district, 5);
  return (
    <div className="mt-8">
      <div className="rounded-2xl border border-line bg-white px-6 py-10 text-center">
        <p className="font-display text-xl font-medium text-[color:var(--pp-primary-950)]">
          {tx("There are no facilities available in this district. You can check another district nearby.")}
        </p>
      </div>
...
function DirectoryCard({
  ...
  const { tx } = useI18n();
  const n = normalizeHfCode(facility.hfCode) || String(facility.hfCode);
  const claim = activated ? getFacilityClaim(n) : null;
  const live = Boolean(activated && claim?.published);
  const faded = !live;
  const name = live ? displayFacilityName(claim?.name || facility.name) : maskFacilityName(facility.name);
  const district = (live && claim ? claim.district : facility.district) || "Nepal";
  const kind = displayFacilityLevel(claim?.facilityLevel || facility.facilityLevel);
