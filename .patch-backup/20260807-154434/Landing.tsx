...
/** Circled arrow used on CTAs. `filled` = solid dark disc with white arrow. */
function ArrowCircle({ size = 26, filled = false, className = "" }: { size?: number; filled?: boolean; className?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 28 28" fill="none" className={className} aria-hidden>
      {filled
        ...
        : <circle cx="14" cy="14" r="12.6" stroke="currentColor" strokeWidth="1.3" />}
      <path d="M10.4 14h7M14.4 11l3 3-3 3" stroke={filled ? "var(--surface-2)" : "currentColor"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function Header() {
  ...
  return (
    <div className="sticky top-3 z-40 px-5 sm:px-8">
      <div
        className="mx-auto flex h-14 w-full max-w-[1280px] items-center justify-between gap-4 rounded-2xl px-5 backdrop-blur-md sm:px-6"
...
function Hero() {
  ...
  return (
    <section className="relative -mt-[68px] h-[560px] overflow-hidden bg-[color:var(--pp-lavender)] sm:h-[660px]">
      ...
      {/* corner labels */}
      <div className="absolute inset-x-0 bottom-5 z-10 mx-auto flex w-full max-w-[1280px] flex-wrap items-center justify-between gap-2 px-5 text-[11px] font-medium text-[color:var(--pp-navy)] sm:px-8">
...
function Welcome({ onStart }: { onStart: () => void }) {
  ...
  return (
    <section className="relative z-20 -mt-16 px-5 sm:px-8">
      <div className="mx-auto w-full max-w-[1280px] rounded-t-[28px] bg-surface-2 px-6 pb-12 pt-9 text-center sm:px-14">
...
function BuyAgain({ go }: { go: (to?: string) => void }) {
  return (
    <section className="mx-auto w-full max-w-[1280px] px-5 pt-14 sm:px-8">
      <div className="grid gap-6 lg:grid-cols-[1.08fr_1fr] lg:gap-8">
        {/* promo */}
        <div>
          <SectionHeads title="Buy again!" onLink={() => go("/messages")} />
          <div
            className="relative flex min-h-[300px] items-center overflow-hidden rounded-[24px] p-8 sm:min-h-[380px] sm:p-10"
            style={{ backgroundImage: "linear-gradient(135deg,#9E86EE 0%,#8A6FE3 42%,#6F55CE 100%)" }}
          >
            <div className="relative z-10">
              <h3 className="font-display text-[34px] font-light leading-[1.12] text-white sm:text-[46px]">
                Ozempic<sup className="text-[0.45em] font-normal align-super">®</sup> now<br />at just $139
              </h3>
              <button
                onClick={() => go("/drug/ozempic")}
                className="mt-8 inline-flex items-center gap-3 rounded-full bg-white py-3 pl-7 pr-3 text-[15px] font-medium text-[color:var(--pp-headline)] transition-transform hover:scale-[1.02]"
              >
                Order now <ArrowCircle size={28} />
              </button>
            </div>

            {/* radiating dotted rings + injector pen */}
            <div className="pointer-events-none absolute right-0 top-0 h-full w-1/2" aria-hidden>
              {[0.42, 0.62, 0.82, 1.02].map((k) => (
                <span key={k} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-dashed border-white/25"
                  style={{ width: `${k * 100}%`, aspectRatio: "1" }} />
              ))}
              <div className="absolute left-1/2 top-1/2 h-[85%] w-[16%] -translate-x-1/2 -translate-y-1/2 rotate-[18deg]">
                <div className="h-full w-full rounded-full bg-gradient-to-b from-[#9BB6D8] via-[#B8CCE4] to-[#E8EDF3] shadow-float">
                  <span className="absolute inset-x-[22%] top-[4%] h-[9%] rounded-full bg-[#7E9BC0]" />
                  <span className="absolute inset-x-0 top-[52%] h-[3%] bg-[#D9C48F]" />
                  <span className="absolute inset-x-0 bottom-0 h-[34%] rounded-b-full bg-gradient-to-b from-white to-[#F3F4F6]" />
                  <span className="absolute inset-x-0 bottom-[6%] h-[16%] bg-[#E85D6B]" />
                </div>
              </div>
            </div>
          </div>
...
function FeatureCards({ go }: { go: (to?: string) => void }) {
  return (
    <section className="mx-auto w-full max-w-[1280px] px-5 pt-12 sm:px-8">
      <div className="grid gap-5 md:grid-cols-3">
        {/* 1 — light lavender + DNA */}
        <button onClick={() => go("/drug/ozempic")}
          className="relative flex h-[300px] flex-col justify-between overflow-hidden rounded-[20px] bg-[#F6F4FE] p-7 text-left transition-transform hover:-translate-y-0.5 sm:h-[360px]">
          <p className="relative z-10 max-w-[62%] font-display text-[20px] font-medium leading-snug text-[color:var(--pp-headline)]">Is Ozempic right for you?</p>
          <span className="relative z-10 inline-flex items-center gap-2.5 text-[15px] text-[color:var(--pp-headline)]">Learn more <ArrowCircle size={26} filled className="text-[color:var(--pp-headline)]" /></span>
          <svg className="pointer-events-none absolute -left-6 top-1/4 h-2/3 w-40 opacity-40" viewBox="0 0 100 200" fill="none" stroke="#A5A0D3" strokeWidth="1.5" aria-hidden>
            {[0, 1, 2, 3].map((i) => <path key={i} d={`M20 ${i * 50} C 80 ${i * 50 + 25}, 80 ${i * 50 + 25}, 20 ${i * 50 + 50}`} />)}
            {[0, 1, 2, 3].map((i) => <path key={"b" + i} d={`M80 ${i * 50} C 20 ${i * 50 + 25}, 20 ${i * 50 + 25}, 80 ${i * 50 + 50}`} />)}
          </svg>
          <Avatar seed="woman" className="pointer-events-none absolute bottom-0 right-0 h-[72%] w-[46%] rounded-tl-[40px]" />
        </button>

        {/* 2 — deep purple */}
        <button onClick={() => go("/find-care")}
          className="relative flex h-[300px] flex-col justify-between overflow-hidden rounded-[20px] bg-[#5B2E9D] p-7 text-left transition-transform hover:-translate-y-0.5 sm:h-[360px]">
          <p className="relative z-10 max-w-[70%] font-display text-[20px] font-medium leading-snug text-white">Get a Sildenafil prescription.</p>
          <span className="relative z-10 inline-flex items-center gap-2.5 text-[15px] text-white">Learn more <ArrowCircle size={26} filled className="text-white" /></span>
          <svg className="pointer-events-none absolute inset-0 h-full w-full opacity-70" viewBox="0 0 300 360" fill="none" aria-hidden>
            <path d="M20 120 C 120 150, 190 250, 300 330" stroke="rgba(255,255,255,.45)" strokeWidth="1.2" />
            {[[38, 126], [126, 176], [216, 262]].map(([cx, cy]) => <circle key={cx} cx={cx} cy={cy} r="4.5" fill="#C4B5FD" />)}
          </svg>
          <Avatar seed="man" className="pointer-events-none absolute bottom-0 right-0 h-[74%] w-[52%] rounded-tl-[40px]" />
        </button>

        {/* 3 — partnership / mountain */}
        <button onClick={() => go("/find-care")}
          className="relative flex h-[300px] flex-col justify-between overflow-hidden rounded-[20px] bg-[#E5E4F6] p-7 text-left transition-transform hover:-translate-y-0.5 sm:h-[360px]">
          <p className="relative z-10 max-w-[80%] font-display text-[20px] font-medium leading-snug text-[color:var(--pp-headline)]">
            Pocketpills × Novo Nordisk<br />Changing care for Canadians.
          </p>
          <span className="relative z-10 inline-flex items-center gap-2.5 text-[15px] text-[color:var(--pp-headline)]">Our mission <ArrowCircle size={26} filled className="text-[color:var(--pp-headline)]" /></span>
          <span className="pointer-events-none absolute inset-x-0 bottom-0 h-[58%]" style={{ backgroundImage: "linear-gradient(180deg,#F3C98B 0%,#D9A9C0 22%,#8FA8CE 46%,#5C7FA8 100%)" }} aria-hidden />
          <span className="pointer-events-none absolute inset-x-0 bottom-0 h-[46%]" style={{ backgroundImage: "linear-gradient(160deg,#8D9DB8,#3F5876)", clipPath: "polygon(0 62%,14% 26%,28% 54%,44% 12%,62% 48%,78% 20%,100% 58%,100% 100%,0 100%)" }} aria-hidden />
          <div className="pointer-events-none absolute bottom-5 right-4 z-10 flex flex-col items-end gap-2">
            <span className="rounded-lg bg-white/85 px-2.5 py-1 text-[10px] font-bold text-[color:var(--pp-navy)] backdrop-blur">pocketpills</span>
            <span className="rounded-lg bg-white/85 px-2.5 py-1 text-[10px] font-bold text-[#1e6bb8] backdrop-blur">novo nordisk</span>
          </div>
        </button>
      </div>
    </section>
...
function Testimonials() {
  ...
  return (
    <section id="reviews" className="mx-auto w-full max-w-[1280px] px-5 py-10 sm:px-8">
...
function NabpBand() {
  return (
    <section className="mx-auto w-full max-w-[1280px] px-5 py-10 sm:px-8">
...
function Faq({ go }: { go: (to?: string) => void }) {
  ...
  return (
    <section id="faq" className="mx-auto w-full max-w-[1280px] px-5 py-10 sm:px-8">
...
function AppAndCare({ go }: { go: (to?: string) => void }) {
  return (
    <section id="care" className="mx-auto w-full max-w-[1280px] px-5 py-10 sm:px-8">
...
function DeliversTo() {
  ...
  return (
    <section id="delivers" className="mx-auto w-full max-w-[1280px] px-5 py-10 sm:px-8">
  ...
  return (
    <footer className="bg-surface-1">
      ...
      <div className="border-t border-line">
        <div className="mx-auto flex w-full max-w-[1280px] flex-col justify-between gap-2 px-5 py-4 text-[11px] text-ink-tertiary sm:flex-row sm:px-8">
...
export function Landing() {
  ...
  return (
    <div className="min-h-screen bg-surface-2">
      ...
      <AppAndCare go={go} />
      <DeliversTo />
      <section className="mx-auto w-full max-w-[1280px] px-5 pb-6 sm:px-8">
