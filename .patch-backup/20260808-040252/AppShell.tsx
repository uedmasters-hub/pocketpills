    ...
    <aside className="hidden w-60 shrink-0 lg:block">
      <nav className="sticky top-28 flex flex-col gap-1">
        ...
        <div className="relative">
          ...
          {openMore && (
            <>
              <div className="fixed inset-0 z-10" onClick={() => setOpenMore(false)} aria-hidden />
...
function MobileNav() {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-line bg-surface-1/95 backdrop-blur lg:hidden">
  ...
  return (
    <div className="min-h-screen bg-surface-0">
      ...
      <div className="mx-auto flex w-full max-w-[105rem] gap-8 px-5 pb-28 pt-8 md:px-8 lg:pb-16 xl:px-20">
        <Sidebar />
        {/* Narrow, centred reading column — the signed-in content measure. */}
        <main className="mx-auto w-full min-w-0 max-w-[45rem]">{children}</main>
      </div>
      <MobileNav />
    </div>
  );
