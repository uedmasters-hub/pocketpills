...
function TileIcon({ id, color }: { id: EntryIconKey; color: string }) {
  const c = { width: 22, height: 22, viewBox: "0 0 24 24", fill: "none", stroke: color, strokeWidth: 1.8, strokeLinecap: "round" as const, strokeLinejoin: "round" as const };
...
function Tiles({ onPick, last }: { onPick: (to: string) => void; last?: { title: string; to: string } }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {items.map((e) => (
        <button key={e.title} onClick={() => onPick(e.to)}
          className="rounded-2xl bg-surface-1 p-4 text-left transition-all hover:-translate-y-0.5 hover:shadow-card">
          <span className="grid h-10 w-10 place-items-center rounded-xl shadow-sm" style={{ backgroundImage: e.tile }}>
            <TileIcon id={e.id} color={e.fg} />
          </span>
          <p className="mt-8 text-sm font-medium leading-snug text-ink">{e.title}</p>
        </button>
      ))}
    </div>
  );
...
function Avatar({ seed, className = "" }: { seed: string; className?: string }) {
  ...
  return (
    <div className={"relative overflow-hidden " + className} style={{ background: `linear-gradient(150deg, ${h}, #322e6b)` }} aria-hidden>
      <div className="absolute inset-0 grid place-items-end justify-center">
        <div className="mb-0 h-1/2 w-1/2 rounded-t-full bg-white/25" />
      </div>
...
function AnnouncementBar({ onGo }: { onGo: () => void }) {
  ...
  if (!show) return null;
  return (
    <div className="relative bg-[#231f4d] text-white">
      <div className="mx-auto flex max-w-7xl items-center justify-center gap-3 px-10 py-2 text-xs">
        <span className="text-white/85">Ozempic® now at just $139</span>
        <button onClick={onGo} className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 font-semibold hover:bg-white/25">
          Get started <span aria-hidden>{PLUS}</span>
        </button>
        <button onClick={() => setShow(false)} className="absolute right-5 text-white/60 hover:text-white" aria-label="Dismiss">✕</button>
      </div>
    </div>
  );
}
...
function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-surface-2/95 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-4 px-5 sm:px-8">
        <Link to="/" className="flex items-center gap-2">
          <span className="grid h-7 w-7 place-items-center rounded-lg bg-primary text-sm font-bold text-[color:var(--color-primary-fg)]">p</span>
          <span className="font-display text-base font-extrabold tracking-tight text-ink">pocketpills</span>
        </Link>
        ...
        <nav className="hidden items-center gap-1 lg:flex" onMouseLeave={() => setOpen(null)}>
          {Object.keys(MENUS).map((m) => (
            <div key={m} className="relative" onMouseEnter={() => setOpen(m)}>
              <button className="flex items-center gap-1 rounded-lg px-3 py-2 text-[13px] font-medium text-ink-secondary hover:text-ink" aria-expanded={open === m}>
                {m}<span className="text-[9px] text-ink-tertiary" aria-hidden>▼</span>
              </button>
              {open === m && (
        ...
        <div className="flex items-center gap-3">
          {signedIn ? (
            <Link to="/app" className="inline-flex h-9 items-center rounded-full bg-[#231f4d] px-4 text-[13px] font-semibold text-white">Continue as {displayName}</Link>
          ) : (
            <>
              <Link to="/login" className="hidden items-center gap-1.5 text-[13px] font-medium text-ink-secondary hover:text-ink sm:inline-flex">
              ...
              <Link to="/get-started" className="inline-flex h-9 items-center rounded-full bg-[#231f4d] px-4 text-[13px] font-semibold text-white hover:bg-[#322e6b]">Join Pocketpills</Link>
            </>
          )}
        </div>
      </div>
...
/* ═══ 3. Hero — autoplaying background video ════════════ */
function Hero() {
  ...
  return (
    <section className="relative bg-[#EAE6FB] pb-16">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="relative mx-auto aspect-[4/3] w-full max-w-2xl overflow-hidden bg-[#322e6b] sm:aspect-[16/11]">
          <iframe
            ref={ref}
            ...
            allowFullScreen
            className="pointer-events-none absolute left-1/2 top-1/2 h-[240%] w-[240%] -translate-x-1/2 -translate-y-1/2 sm:h-[190%] sm:w-[190%]"
          />
          <button onClick={() => { cmd(playing ? "pauseVideo" : "playVideo"); setPlaying(!playing); }}
            className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-full bg-black/35 text-[11px] text-white backdrop-blur hover:bg-black/55"
      ...
      <div className="pointer-events-none absolute bottom-14 left-1/2 flex -translate-x-1/2 gap-2" aria-hidden>
        <span className="h-6 w-16 rounded-full bg-[#7C74BC] opacity-80" />
        <span className="h-6 w-10 rounded-full bg-[#2DD4BF] opacity-80" />
        <span className="h-6 w-14 rounded-full bg-[#A5A0D3] opacity-80" />
      </div>

      {/* corner labels */}
      <div className="mx-auto mt-4 flex max-w-7xl flex-wrap items-center justify-between gap-2 px-5 text-[11px] font-medium text-[#3E3985] sm:px-8">
        <span><span className="mr-1" aria-hidden>🍁</span>Complete care, without leaving home</span>
...
function Welcome({ onStart }: { onStart: () => void }) {
  ...
  return (
    <section className="relative z-10 -mt-10 px-5 sm:px-8">
      <div className="mx-auto max-w-5xl rounded-t-3xl bg-surface-2 px-6 pb-10 pt-10 text-center sm:px-10">
        <p className="text-sm font-semibold text-primary">Welcome to Pocketpills</p>
        <h1 className="mt-3 font-display text-4xl font-extrabold tracking-tight text-[#2b2560] sm:text-5xl">Your health, handled.</h1>
        <button onClick={onStart} className="mt-7 inline-flex items-center gap-2 rounded-full bg-[#3b3475] px-7 py-3 text-sm font-semibold text-white hover:bg-[#2b2560]">
          Start <span aria-hidden>{PLUS}</span>
        </button>
        <div className="mt-10 grid grid-cols-2 gap-y-6 border-t border-line pt-6 text-left sm:grid-cols-3 lg:grid-cols-5 lg:divide-x lg:divide-line">
          {stats.map(([big, small], i) => (
            <div key={small} className={i > 0 ? "lg:pl-5" : ""}>
              <p className="text-[13px] font-bold text-ink">{big}</p>
              <p className="mt-0.5 text-[11px] leading-snug text-ink-tertiary">{small}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <div>
          <div className="mb-3 flex items-center gap-3">
            <p className="text-sm font-bold text-ink">Buy again!</p>
            <button onClick={() => go("/messages")} className="text-xs font-medium text-ink-secondary hover:text-ink">Talk to a licensed clinician ›</button>
          </div>
          <div className="relative flex min-h-[190px] items-center overflow-hidden rounded-2xl p-7 text-white"
            style={{ backgroundImage: "radial-gradient(320px 220px at 78% 45%, rgba(199,195,229,.45), transparent 65%), linear-gradient(120deg,#6d5fc4,#4a44a0)" }}>
            <div className="relative z-10">
              <h3 className="font-display text-2xl font-bold leading-snug">Ozempic® now<br />at just $139</h3>
              <button onClick={() => go("/drug/ozempic")} className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-white px-4 py-2 text-xs font-semibold text-[#3b3475]">
                Order now <span aria-hidden>{PLUS}</span>
              </button>
            </div>
            <div className="pointer-events-none absolute right-6 top-1/2 h-40 w-14 -translate-y-1/2 rotate-12 rounded-full bg-gradient-to-b from-white via-white to-stone-200 shadow-float" aria-hidden>
              <span className="absolute inset-x-2 top-3 h-16 rounded-full bg-[#dfe6ee]" />
              <span className="absolute inset-x-3 bottom-6 h-8 rounded-sm bg-[#1e6bb8]" />
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="mb-3 flex items-center gap-3">
            <p className="text-sm font-bold text-ink">Doctor-led treatment</p>
            <button onClick={() => go("/find-care")} className="text-xs font-medium text-ink-secondary hover:text-ink">Talk to a licensed clinician ›</button>
          </div>
...
function FeatureCards({ go }: { go: (to?: string) => void }) {
  const cards = [
    { t: "Is Ozempic right for you?", cta: "Learn more", bg: "bg-[#EAE6FB]", fg: "text-ink", to: "/drug/ozempic", art: "dna" },
    { t: "Get a Sildenafil prescription.", cta: "Learn more", bg: "bg-[#5b52ad] text-white", fg: "text-white", to: "/find-care", art: "dots" },
    { t: "Pocketpills × Novo Nordisk", sub: "Changing care for Canadians.", cta: "Our mission", bg: "bg-[#cfe0f5]", fg: "text-[#1c3d63]", to: "/find-care", art: "mtn" },
  ];
  return (
    <section className="mx-auto max-w-5xl px-5 pt-6 sm:px-8">
      <div className="grid gap-4 md:grid-cols-3">
        {cards.map((c) => (
          <button key={c.t} onClick={() => go(c.to)} className={`relative flex h-44 flex-col justify-between overflow-hidden rounded-2xl p-5 text-left ${c.bg} transition-transform hover:-translate-y-0.5`}>
            <div className="relative z-10 max-w-[62%]">
              <p className={`font-display text-sm font-bold leading-snug ${c.fg}`}>{c.t}</p>
              {c.sub && <p className={`mt-1 text-sm font-bold leading-snug ${c.fg}`}>{c.sub}</p>}
            </div>
            <span className={`relative z-10 inline-flex items-center gap-1.5 text-xs font-semibold ${c.fg}`}>{c.cta} <span aria-hidden>{PLUS}</span></span>
            {c.art === "dna" && <span className="pointer-events-none absolute -left-4 bottom-0 h-32 w-20 rounded-full border-4 border-dashed border-[#A5A0D3]/50" aria-hidden />}
            {c.art === "dots" && <span className="pointer-events-none absolute inset-0 opacity-30" style={{ backgroundImage: "radial-gradient(rgba(255,255,255,.9) 1.5px, transparent 1.5px)", backgroundSize: "26px 26px" }} aria-hidden />}
            {c.art === "mtn" && <span className="pointer-events-none absolute inset-x-0 bottom-0 h-16" style={{ backgroundImage: "linear-gradient(160deg,#7fa8d4,#2f5d8c)", clipPath: "polygon(0 60%,25% 20%,45% 55%,70% 10%,100% 55%,100% 100%,0 100%)" }} aria-hidden />}
            <Avatar seed={c.t} className="pointer-events-none absolute -bottom-2 right-0 h-28 w-24 rounded-tl-3xl" />
          </button>
        ))}
      </div>
    </section>
...
/* ═══ 7. Partner marquee ════════════════════════════════ */
function Partners() {
  const logos = ["KOHO", "indeed", "123DENTIST", "VetCare", "CANADIAN TIRE", "TOYOTA", "neo", "Novo Nordisk"];
  return (
    <section className="mt-16 overflow-hidden py-8">
      <p className="mb-6 text-center text-[11px] font-medium text-ink-tertiary">Proud pharmacy of:</p>
      <div className="relative">
        <div className="pp-marquee flex w-max gap-14">
          {[...logos, ...logos].map((l, i) => (
            <span key={i} className="shrink-0 font-display text-base font-bold tracking-wide text-ink-tertiary/70">{l}</span>
          ))}
        </div>
      </div>
    </section>
...
function Testimonials() {
  ...
  return (
    <section id="reviews" className="mx-auto max-w-6xl px-5 py-10 sm:px-8">
      <h2 className="text-center font-display text-3xl font-extrabold text-[#2b2560]">Our members love us</h2>
        ...
        <div ref={box} className="pp-scroll flex gap-4 overflow-x-auto px-1 pb-2">
          {reviews.map((r) => (
            <div key={r.n} className="pp-snap w-[250px] shrink-0 rounded-2xl border border-line bg-surface-2 p-5">
              <span className="text-xs tracking-widest text-[#5b52ad]" aria-label="5 out of 5">★★★★★</span>
  ...
  return (
    <section className="py-10">
      <div className="mx-auto max-w-3xl px-5 text-center sm:px-8">
        <h2 className="font-display text-3xl font-extrabold leading-snug text-[#2b2560] sm:text-4xl">
          Join <span className="text-primary">800,000+</span> Canadians<br />who never miss a dose.
        </h2>
        <button onClick={() => go()} className="mt-6 rounded-full bg-[#3b3475] px-6 py-2.5 text-sm font-semibold text-white hover:bg-[#2b2560]">Join Pocketpills</button>
      </div>

      <div className="pp-scroll mt-10 flex gap-3 overflow-x-auto px-5 pb-2 sm:px-8">
        {members.map((m, i) => (
          <div key={m.n} className={"pp-snap h-52 w-64 shrink-0 overflow-hidden rounded-2xl " + (m.card ? "bg-[#5b52ad] p-5 text-white" : "relative")}>
            {m.card ? (
              <>
...
function NabpBand() {
  return (
    <section className="mx-auto max-w-5xl px-5 py-10 sm:px-8">
      <div className="flex flex-col gap-6 rounded-2xl bg-[#0f4c3f] p-8 text-white sm:flex-row sm:items-center sm:p-10">
...
function Faq({ go }: { go: (to?: string) => void }) {
  ...
  return (
    <section id="faq" className="mx-auto max-w-5xl px-5 py-10 sm:px-8">
      <div className="grid gap-8 lg:grid-cols-[260px_1fr]">
        <div>
          <p className="text-sm font-semibold text-primary">Frequently Asked</p>
          <h2 className="mt-1.5 font-display text-2xl font-extrabold leading-snug text-[#2b2560]">Your questions, answered.</h2>
          <button onClick={() => go()} className="mt-4 rounded-full bg-[#3b3475] px-4 py-2 text-xs font-semibold text-white">More FAQs</button>
        </div>
...
function AppAndCare({ go }: { go: (to?: string) => void }) {
  return (
    <section id="care" className="mx-auto max-w-5xl px-5 py-10 sm:px-8">
...
function DeliversTo() {
  ...
  return (
    <section id="delivers" className="mx-auto max-w-5xl px-5 py-10 sm:px-8">
  ...
  return (
    <footer className="bg-surface-1">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-line pt-6">
          ...
          <div className="text-right">
            ...
            <div className="mt-1.5 flex gap-2">
              <span className="grid h-9 w-9 place-items-center rounded-full bg-[#0f4c3f] text-[7px] font-bold text-white">NABP</span>
      ...
      <div className="border-t border-line">
        <div className="mx-auto flex max-w-5xl flex-col justify-between gap-2 px-5 py-4 text-[11px] text-ink-tertiary sm:flex-row sm:px-8">
