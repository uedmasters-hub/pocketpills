        ...
        <>
          ...
          <div role="menu" className="absolute right-0 z-20 mt-2 w-60 overflow-hidden rounded-2xl border border-line bg-surface-2 shadow-float">
            ...
            <button role="menuitem" onClick={() => setDark((d) => !d)}
              className="flex w-full items-center justify-between border-t border-line px-4 py-2.5 text-left text-[13px] font-medium text-ink-secondary hover:bg-surface-1">
              {dark ? "Light mode" : "Dark mode"} <span aria-hidden>{dark ? "☀️" : "🌙"}</span>
            </button>
            <button role="menuitem" onClick={() => { setOpen(false); logOut(); nav("/"); }}
              className="block w-full border-t border-line px-4 py-2.5 text-left text-[13px] font-semibold text-danger hover:bg-surface-1">
              Sign out
            </button>
          </div>
        </>
  ...
  return (
    <Shell>
      ...
      <nav className="hidden items-center gap-1 md:flex" onMouseLeave={() => setOpen(null)}>
        {isApp ? (
          <>
            {[["My Health", "/dashboard"], ["Pharmacy", "/pharmacy"], ["Orders", "/orders"], ["Messages", "/messages"]].map(([l, to]) => (
              <NavLink key={to} to={to} className={({ isActive }) => ITEM + (isActive ? " bg-[color:var(--pp-primary-100)]" : "")}>
                {l}
              </NavLink>
            ))}
            <div className="relative" onMouseEnter={() => setOpen("p")}>
              <span className={ITEM}><Link to="/drug">Explore</Link><Chevron /></span>
              {open === "p" && <PharmacyMenu />}
            </div>
          </>
        ) : (
