...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const browse =
    pathname.startsWith("/drug") || pathname.startsWith("/find-care") || pathname.startsWith("/treatment");
  ...
  return (
    <div className="min-h-screen bg-surface-0">
      ...
      <div className="mx-auto flex w-full max-w-[105rem] gap-8 px-5 pb-28 pt-8 md:px-8 lg:pb-16 xl:px-20">
        <Sidebar />
        {/* Two measures: a narrow reading column for personal screens, and a
            wider canvas for browse/reference pages that carry filters + grids. */}
        <main className={"mx-auto w-full min-w-0 " + (browse ? "max-w-[68rem]" : "max-w-[45rem]")}>
          {children}
        </main>
      </div>
