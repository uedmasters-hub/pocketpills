...
function FeatureCard({
  onClick, bg, titleClass, cta, ctaClass, children, art,
}: {
  onClick: () => void; bg: string; titleClass: string; cta: string; ctaClass: string;
  children: React.ReactNode; art: React.ReactNode;
}) {
  return (
    <button onClick={onClick}
      className={"group relative aspect-[7/5] min-h-[260px] overflow-hidden rounded-[20px] text-left transition-transform hover:-translate-y-0.5 " + bg}>
      {art}
      <p className={"absolute left-7 right-7 top-7 z-10 font-display text-[clamp(16px,1.5vw,21px)] font-medium leading-snug " + titleClass}>
        {children}
      </p>
      <span className={"absolute bottom-7 left-7 z-10 inline-flex items-center gap-2.5 text-[15px] " + ctaClass}>
        {cta} <ArrowCircle size={26} filled className={ctaClass} />
      </span>
    </button>
  );
}
    ...
    <section className="mx-auto w-full max-w-[1280px] px-5 pt-12 sm:px-8">
      <div className="grid gap-5 md:grid-cols-3">
        <FeatureCard
          onClick={() => go("/drug/ozempic")}
          bg="bg-[#F6F4FE]"
          titleClass="text-[color:var(--pp-headline)] max-w-[58%]"
          cta="Learn more"
          ctaClass="text-[color:var(--pp-headline)]"
          art={
            <>
              <svg className="pointer-events-none absolute -left-4 bottom-0 h-[72%] w-32 opacity-35" viewBox="0 0 100 200" fill="none" stroke="#A5A0D3" strokeWidth="1.4" aria-hidden>
              ...
              <img src={IMG.ozempicCard} alt="" loading="lazy" onError={hideOnError}
                className="pointer-events-none absolute bottom-0 right-0 h-[88%] w-auto max-w-[58%] object-contain object-bottom" />
            </>
          }
        >
        ...
        <FeatureCard
          ...
          bg="bg-[#5B2E9D]"
          titleClass="text-white max-w-[62%]"
          cta="Learn more"
          ctaClass="text-white"
          art={
            <>
              <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 400 290" fill="none" preserveAspectRatio="none" aria-hidden>
                <path d="M30 110 C 150 145, 230 215, 400 285" stroke="rgba(255,255,255,.4)" strokeWidth="1.1" />
                {[[46, 114], [150, 158], [268, 218]].map(([cx, cy]) => <circle key={cx} cx={cx} cy={cy} r="4" fill="#C4B5FD" />)}
              </svg>
              <img src={IMG.sildenafilCard} alt="" loading="lazy" onError={hideOnError}
                className="pointer-events-none absolute bottom-0 right-0 h-[90%] w-auto max-w-[62%] object-contain object-bottom" />
            </>
          }
        >
        ...
        <FeatureCard
          onClick={() => go("/find-care")}
          bg="bg-[#E5E4F6]"
          titleClass="text-[color:var(--pp-headline)] max-w-[80%]"
          cta="Our mission"
          ctaClass="text-[color:var(--pp-headline)]"
          art={
            <>
              ...
              <span className="pointer-events-none absolute inset-x-0 bottom-0 h-[44%]" style={{ backgroundImage: "linear-gradient(160deg,#8D9DB8,#3F5876)", clipPath: "polygon(0 62%,14% 26%,28% 54%,44% 12%,62% 48%,78% 20%,100% 58%,100% 100%,0 100%)" }} aria-hidden />
              <img src={IMG.novoCard} alt="" loading="lazy" onError={hideOnError}
