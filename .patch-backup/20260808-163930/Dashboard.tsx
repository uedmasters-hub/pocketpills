        ...
        <div ref={treatRef} className="pp-scroll flex gap-4 overflow-x-auto pb-1">
          {treatments.map((t) => (
            <button key={t.slug} onClick={() => nav(`/treatment/${t.slug}`)}
              className="pp-snap w-[11.5rem] shrink-0 text-left">
              <p className="mb-3 min-h-[3.25rem] text-[18px] font-medium leading-snug text-[color:var(--pp-primary-950)]">
                {t.name}
              </p>
              <span
                className="relative block h-[16.5rem] w-full overflow-hidden rounded-2xl"
                style={{ backgroundImage: "linear-gradient(180deg,#FBFAFE 0%,#EFECFA 55%,#DED8F5 100%)" }}
              >
                {t.img ? (
                  <img
                    src={t.img}
                  ...
                  <span className="absolute inset-0 grid place-items-center text-4xl" aria-hidden>{t.emoji}</span>
                )}
              </span>
            </button>
          ))}
