...
import { profileChecklist, type ChecklistId } from "@/lib/profile";

const IDS: ChecklistId[] = ["personal", "health", "card", "insurance", "shipping", "payment"];
...
export function ProfileSection() {
  ...
  const id = (IDS as string[]).includes(section ?? "") ? (section as ChecklistId) : null;
  if (!id) {
    return (
      <div className={`${CARD} text-center`}>
        <p className="font-semibold text-[color:var(--pp-primary-950)]">Section not found</p>
        <Link to="/profile" className="mt-2 inline-block text-[14px] font-semibold text-[color:var(--pp-violet)] hover:underline">Back to profile</Link>
      </div>
    );
  }
