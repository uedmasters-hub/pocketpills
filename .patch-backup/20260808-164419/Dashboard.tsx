        ...
        <div ref={treatRef} className="pp-scroll flex gap-4 overflow-x-auto pb-1">
          {treatments.map((t) => (
            <button
              ...
              className="pp-snap relative aspect-[4/5] w-[13.5rem] shrink-0 overflow-hidden rounded-2xl text-left transition-transform hover:-translate-y-0.5"
              style={{ backgroundImage: "linear-gradient(165deg,#FCFBFE 0%,#F1EEFB 45%,#E2DCF7 100%)" }}
            >
              {/* portrait bleeds off the lower-right; the top-left stays clear for the label */}
              {t.img ? (
                <img
                  src={t.img}
                  alt=""
                  loading="lazy"
                  onError={hideOnError}
                  className="pointer-events-none absolute -right-3 bottom-0 h-[78%] w-auto max-w-none object-contain object-bottom"
                />
              ) : (
                <span className="pointer-events-none absolute bottom-6 right-5 text-5xl opacity-90" aria-hidden>{t.emoji}</span>
              )}

              <p className="relative z-10 px-5 pt-5 font-display text-[19px] font-medium leading-snug text-[color:var(--pp-primary-950)]">
                {t.name}
              </p>
            </button>
          ))}
